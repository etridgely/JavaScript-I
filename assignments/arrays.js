...
let forgottenCar = inventory[32];
console.log("Car 33 is a " + forgottenCar.car_year + " " + forgottenCar.car_make + " " + forgottenCar.car_model + ".");
...
for (let i = 0; i < inventory.length; i++) {
    let carID = inventory[i]; 
    carModels.unshift(carID.car_model);
    carModels.sort();
}
...
let carYears = [];
for (let i = 0; i < inventory.length; i++) {
    let carAnnum = inventory[i]; 
    carYears.unshift(carAnnum.car_year);
}
...
for (let i = 0; i < carYears.length; i++) {
    let carMillenium = carYears[i];
    if (carMillenium < 2000) {
        oldCars.unshift(carYears[i]);
    }
}
//Stretch
//let oldCars = carYears.filter(carMillenium => carMillenium < 2000);
...
let BMWAndAudi = [];
for (let i = 0; i < inventory.length; i++) {
    let audi = inventory[i];
    if  (audi.car_make == "Audi") {
        BMWAndAudi.unshift(audi.car_model);
    }
    if  (audi.car_make == "BMW") {
        BMWAndAudi.unshift(audi.car_model);
    }
 }
